...
WITH staging AS (
  ...
  FROM DELTA_SCAN("./lakehouse/bronze/raw__adventure_works__product_list_price_histories")
), validity AS (
  SELECT
    *,
  ...
  SELECT
    CONCAT('product|adventure_works|', product_id, '~epoch|valid_from|', product__valid_from)::BLOB AS _pit_hook__product,
