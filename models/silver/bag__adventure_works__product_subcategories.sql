...
WITH staging AS (
  ...
  SELECT
    *,
    ROW_NUMBER() OVER (PARTITION BY product_subcategory_id ORDER BY _sqlmesh__loaded_at) AS _sqlmesh__version,
    CASE
    ...
    COALESCE(
      LEAD(_sqlmesh__loaded_at) OVER (PARTITION BY product_subcategory_id ORDER BY _sqlmesh__loaded_at),
