...
WITH staging AS (
  ...
  SELECT
    *,
    ROW_NUMBER() OVER (PARTITION BY business_entity_id ORDER BY sales_person__record_loaded_at) AS sales_person__record_version,
    CASE
    ...
    COALESCE(
      LEAD(sales_person__record_loaded_at) OVER (PARTITION BY business_entity_id ORDER BY sales_person__record_loaded_at),
  ...
  SELECT
    CONCAT('sales_person|adventure_works|', business_entity_id)::BLOB AS _hook__sales_person,
